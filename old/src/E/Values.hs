...
eCaseTup e vs w = caseUpdate emptyCase { eCaseScrutinee = e
                                       ...
                                       , eCaseType = getType w
                                       , eCaseAlts = [Alt litCons { litName = nameTuple DataConstructor (length vs)
                                                                  , litArgs = vs
                                                                  , litType = getType e } w]
                                       }
...
eCaseTup' e vs w = caseUpdate emptyCase { eCaseScrutinee = e
                                        , eCaseBind = (tVr emptyId (getType e))
                                        , eCaseType = getType w
                                        , eCaseAlts = [Alt litCons { litName = unboxedNameTuple DataConstructor (length vs)
                                                                   , litArgs = vs
                                                                   , litType = getType e} w] }
...
substLet ds e  = ans where
    (as,nas) = partition (isAtomic . snd) (filter ((not . isEmptyId) . tvrIdent . fst) ds)
...
substLet' ds' e  = ans where
    ...
    nas = filter ((not . isEmptyId) . tvrIdent . fst) ds
    tas = filter (sortKindLike . tvrType . fst) nas
    ans = case (nas,tas) of
        ([],_) -> hhh hh $ e
