...
eLetRec :: [(TVr,E)] -> E -> E
eLetRec ds e = f (filter ((not . isEmptyId) . tvrIdent . fst) ds) where
    f [] = e
    f ds = ELetRec ds e
...
doSubst' substInVars allShadow bm check e  = f e (Set.empty, bm) where
    ...
    lp lam tvr@(TVr { tvrIdent = n, tvrType = t}) e | isEmptyId n || (allShadow && n `notElem` freeVars e) = do
        t' <- f t
...
typeSubst termSubst typeSubst e  = f e (False,termSubst',typeSubst) where
    ...
    f ec@(ECase {}) = do
        ...
        return $ caseUpdate ec { eCaseScrutinee = e', eCaseDefault = d, eCaseBind = b', eCaseAlts = alts, eCaseType = nty }
    lp lam tvr@(TVr { tvrIdent = i, tvrType = t}) e  | isEmptyId i = do
        t' <- inType (f t)
        e' <- f e
        return $ lam (tvr { tvrIdent = emptyId, tvrType =  t'}) e'
    lp lam tvr e = do
