module C.FromGrin3(compileGrin) where
...
runC grin (C m) =  execUniq1 (runRWST m Env { rCPR = cpr, rGrin = grin, rDeclare = False, rTodo = TodoExp [], rEMap = mempty, rInscope = mempty } emptyHcHash) where
    ...
    cpr = Set.empty --Set.insert cChar (Set.fromList [ a | (a,TyTy { tySlots = [s], tySiblings = Just [a'] }) <- Map.assocs tmap, a == a', isJust (good s) ])
    good s = do
        ct <- Op.toCmmTy s
        b <- Op.cmmTyBits ct
        guard $ b <= 30
...
convertExp (App a [v] _) | a == funcEval = error "C.FromGrin3.convertExp: All evals should have been removed"
...
declareStruct n = do
    ...
    let (dis,needsDis) | tagIsSuspFunction n = ([(name "head",fptr_t)],False)
        ...
        theStruct = basicStructure {
            ...
            structureNeedsDiscriminator = True -- needsDis
            }
...
f_EVALTAG e   = e -- functionCall (name "EVALTAG") [e]
