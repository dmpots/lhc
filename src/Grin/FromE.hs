...
compile prog@Program { progDataTable = dataTable } = do
    ...
    initTyEnv = mappend primTyEnv $ TyEnv $ Map.fromList $ concat [ makePartials (a,b,c) | (_,(a,b,c)) <-  massocs scMap] ++
                                                           concat [con x| x <- Map.elems $ constructorMap dataTable, conType x /= eHash]
...
constantCaf Program { progDataTable = dataTable, progCombinators = combs } = ans where
    ...
    conv (EPi (TVr { tvrIdent = i, tvrType =  a}) b) | isEmptyId i  =  Const $ NodeC tagArrow [conv a,conv b]
...
compile' cenv (tvr,as,e) = ans where
    ...
    ce ECase { eCaseScrutinee = e, eCaseBind = b, eCaseAlts = as, eCaseDefault = d } |  Just ty <- toCmmTy (getType e :: E) = do
            v <- if isEmptyId (tvrIdent b) then newPrimVar $ TyPrim ty else return $ toVal b
            e <- ce e
            as' <- mapM cp'' as
    ...
    ce ECase { eCaseScrutinee = scrut, eCaseBind = b, eCaseAlts = as, eCaseDefault = d }  = do
        ...
        e <- ce scrut
        case (b,scrut) of
            (TVr { tvrIdent = i },EVar etvr) | isEmptyId i -> localEvaled [etvr] v $ do
                    as <- mapM cp as
    ...
    localEvaled vs v action = local (\lenv -> lenv { evaledMap = nm `mappend` evaledMap lenv }) action where
        nm = fromList [ (tvrIdent x, v) | x <- vs, not (isEmptyId (tvrIdent x)) ]
...
--    constant :: Monad m =>  E -> m Val
    ...
    constant (EPi (TVr { tvrIdent = i, tvrType = a}) b) | isEmptyId i, Just a <- constant a, Just b <- constant b = return $ NodeC tagArrow [a,b]
    constant _ = fail "not a constant term"

    -- | convert a constructor into a Val, arguments may depend on local vars.
    con :: Monad m => E -> m [Val]
    con (EPi (TVr {tvrIdent = i, tvrType = x}) y) | isEmptyId i= do
